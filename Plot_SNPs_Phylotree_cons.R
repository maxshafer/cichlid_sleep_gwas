...
for (i in 1:nrow(genes_for_plotting)) {
  ...
  if(grepl("pref_sfi", pheno_to_test2)) {
    lm <- lm(phenotypes[,"sfi"] ~ phenotypes[,col_name])
    direction <- ifelse(lm$coefficients[2] > 0, 1, -1)
  ...
  if(grepl("cons", pheno_to_test2)) {
    lm <- lm(phenotypes[,"cons"] ~ phenotypes[,col_name])
    direction <- ifelse(lm$coefficients[2] > 0, -1, 1)
  ...
  ggsave(p5, file = paste("../", pheno_to_test2, "_PhyloPlots", "/", pheno_to_test2, "_vs_", col_name, ".pdf", sep = ""))
