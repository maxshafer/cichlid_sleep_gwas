...
gwas.2 <- gwas[gwas$comparison == "day_night_dif",]
for (i in 1:nrow(gwas[gwas$comparison == "day_night_dif",])) {
  gene <- gwas.2$gene[i]
  location <- gwas.2$location[i]
...
gwas.2 <- gwas[gwas$comparison == "total_rest",]
...
phenotypes_p <- phenotypes
gwas.2 <- gwas[gwas$comparison == "peak",]
for (i in 1:nrow(gwas[gwas$comparison == "peak",])) {
  gene <- gwas.2$gene[i]
  location <- gwas.2$location[i]
  chromosome <- str_split(location, ":")[[1]][[1]]
  coord <- as.numeric(str_split(location, ":")[[1]][[2]])
  ...
  phenotypes_p <- phenotypes_p[phenotypes_p$six_letter_name_Ronco %in% names(af2),]
  
  phenotypes_p[,paste("genotype", gene, location, sep = "_")] <- as.numeric(af2)
...
phenotypes_p[phenotypes_p == -1] <- NA
...
dendro_dnd_plot <- dendro_dnd_plot + geom_tile(data = phenotypes_dnd_scaled, aes(y=y, x=8, fill = day_night_dif), width = 1, height = 0.9, size = 1.5, inherit.aes = FALSE) + scale_fill_distiller(palette = "RdBu")
# Preview the plot
# dendro_dnd_plot
...
dendro_tr_plot <- dendro_tr_plot + geom_tile(data = phenotypes_tr_scaled, aes(y=y, x=5.5, fill = total_rest), width = 1, height = 0.9, size = 1.5, inherit.aes = FALSE) + scale_fill_distiller(palette = "PRGn", direction = 1) # + scale_fill_distiller(palette = "RdBu")
# Preview the plot
# dendro_tr_plot
...
phenotypes_p_scaled <- phenotypes_p

# Scale the values, or not
# phenotypes_p_scaled[,c(8:ncol(phenotypes_p_scaled))] <- scale(phenotypes_p_scaled[,c(8:ncol(phenotypes_p_scaled))])

# Run clustering
phenotypes_p_matrix <- as.matrix(phenotypes_p_scaled[, -c(1:7)])
rownames(phenotypes_p_matrix) <- phenotypes_p_scaled$six_letter_name_Ronco
phenotypes_p_dendro <- as.phylo(hclust(d = dist(x = phenotypes_p_matrix)))


# Create dendro
dendro_p_plot <- ggtree(phenotypes_p_dendro) #+ geom_tiplab() 
phenotypes_p_scaled$y <- dendro_p_plot$data$y[match(phenotypes_p_scaled$six_letter_name_Ronco, dendro_p_plot$data$label)]

dendro_p_plot <- dendro_p_plot + geom_tile(data = phenotypes_p_scaled, aes(y=y, x=5.5, fill = peak), width = 1, height = 0.9, size = 1.5, inherit.aes = FALSE) + scale_fill_distiller(palette = "BrBG", direction = 1, limits = c(0,1))
# Preview the plot
# dendro_p_plot
...
phenotypes_p_scaled_r <- t(phenotypes_p)
colnames(phenotypes_p_scaled_r) <- phenotypes_p$six_letter_name_Ronco

# Scale the values, or not
# phenotypes_p_scaled_r[c(8:nrow(phenotypes_p_scaled_r)),] <- scale(as.numeric(phenotypes_p_scaled_r[c(8:nrow(phenotypes_p_scaled_r)),]))

# Run clustering
phenotypes_p_matrix_r <- as.matrix(phenotypes_p_scaled_r[-c(1:7),])
phenotypes_p_dendro_r <- as.phylo(hclust(d = dist(x = phenotypes_p_matrix_r)))

# Create dendro
dendro_p_plot_r <- ggtree(phenotypes_p_dendro_r) + coord_flip()

# Preview the plot
# print(dendro_p_plot_r)



# Heatmap
# Data wrangling
phenotypes_p_long <- pivot_longer(data = phenotypes_p_scaled, cols = -c(six_letter_name_Ronco, total_rest, peak_amplitude, peak, day_night_dif, cluster, day_night_dif_spd, y), names_to = "measurement", values_to = "value")


phenotypes_p_order <- dendro_p_plot$data$label[match(c(1:length(phenotypes_p_dendro$tip.label)), dendro_p_plot$data$y)]
phenotypes_p_order_r <- dendro_p_plot_r$data$label[match(c(1:length(phenotypes_p_dendro_r$tip.label)), dendro_p_plot_r$data$y)]

# Order the levels according to their position in the cluster
phenotypes_p_long$six_letter_name_Ronco <- factor(x = phenotypes_p_long$six_letter_name_Ronco, levels = phenotypes_p_order, ordered = TRUE)

phenotypes_p_long$measurement <- factor(x = phenotypes_p_long$measurement, levels = phenotypes_p_order_r, ordered = TRUE)


heatmap_plot_p <- ggplot(data = phenotypes_p_long, aes(x = measurement, y = six_letter_name_Ronco)) + geom_tile(aes(fill = value)) + theme(axis.text = element_text(size = 6), axis.text.x = element_text(angle = 90), axis.title = element_blank()) + scale_fill_distiller(palette = "Greys", direction = 1) #+ scale_fill_gradient2(midpoint = 0.5)

# Preview the heatmap
# print(heatmap_plot_p)
...
pdf("../../outs/SNP_clustering_day-night-dif.pdf", height = 7.5, width = 7.5)
dendro_dnd_plot + heatmap_plot_dnd + plot_spacer() + dendro_dnd_plot_r + plot_layout(guides = "collect", widths = c(5,10), heights = c(10,5))
dev.off()
pdf("../../outs/SNP_clustering_total-rest.pdf", height = 7.5, width = 7.5)
dendro_tr_plot + heatmap_plot_tr + plot_spacer() + dendro_tr_plot_r + plot_layout(guides = "collect", widths = c(5,10), heights = c(10,5))
dev.off()
pdf("../../outs/SNP_clustering_peak.pdf", height = 7.5, width = 7.5)
dendro_p_plot + heatmap_plot_p + plot_spacer() + dendro_p_plot_r + plot_layout(guides = "collect", widths = c(5,10), heights = c(10,5))
dev.off()
