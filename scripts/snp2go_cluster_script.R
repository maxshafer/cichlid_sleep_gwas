...
dfs_all <- lapply(files, function(x) fread(paste("combined_ann/", x, sep = ""), showProgress = T))
names(dfs_all) <- files
...
########################################################################
filter_snps <- readRDS(here("combined_ann/filter_SNPs_perchr_1e-05_percentile"))

if (args[2] == "pref_sfi") {
  snps <- filter_snps[[4]]
  bkgrd_snps <- bkgrd_snps[!(merged$location %in% filter_snps[[4]]$location)]
}
if (args[2] == "pref_max_cons") {
  snps <- filter_snps[[5]]
...
## Run for pc1
y <- SNP2GO:::snp2go(gtf = "/home/ayasha/projects/def-mshafer/genome/Oreochromis_niloticus.O_niloticus_UMD_NMBU.112.gff3.gz",
                    goFile = args[1],
                    candidateSNPs = snps,
